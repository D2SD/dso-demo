pipeline {
  ...
  environment {
    ARGO_SERVER = 'NODE-IPADDRESS:32100'
    DEV_URL = 'http://NODE-IPADDRESS:30080/'
  }
  stages {
    stage('Build') {
      parallel {
        ...
        stage('OCI Image BnP') {
          steps {
            container('kaniko') {
              ...
              --skip-tls-verify --cache=true --destination=docker.io/d2sd21/dso-demo'
            }
          }
        }
      }
        ...
        stage('Image Linting') {
          steps {
            container('docker-tools') {
              sh 'dockle docker.io/d2sd21/dso-demo'
            }
          }
        }
        stage('Image Scan') {
          steps {
            container('docker-tools') {
              sh 'trivy image --timeout 10m --exit-code 1 d2sd21/dso-demo'
            }
          }
        }
      }
